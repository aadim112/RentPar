...
const AddSpace = (props) => {
  ...
  const handleChange = (e) => {
    ...
    if (type === "checkbox") {
      ...
      // Handle vehicle space count changes
      const vehicleType = name.replace("spaces-", "");
      const numberValue = parseInt(value) || 0;
      
      setSpaceDetails((prevDetail) => ({
        ...prevDetail,
        vehicleSpaces: {
          ...prevDetail.vehicleSpaces,
          [vehicleType]: numberValue
        }
      }));
    } else {
      setSpaceDetails((prevDetail) => ({
  ...
  // Function to render vehicle space inputs based on selected types
  const renderVehicleSpaceInputs = () => {
    return (
      <div className="vehicle-spaces-container" style={{display:'flex',flexDirection:'column',gap:'10px'}}>
        {spacedetails.ParkingType.includes('Twowheels') && (
          <div className="vehicle-space-input">
            <label>Two Wheeler Spaces:</label>
            <input 
              type="number" 
              min="0"
        ...
        {spacedetails.ParkingType.includes('ThreeWheels') && (
          <div className="vehicle-space-input">
            <label>Three Wheeler Spaces:</label>
            <input 
              type="number" 
              min="0"
              placeholder="Number of spaces" 
        ...
        {spacedetails.ParkingType.includes('FourWheels') && (
          <div className="vehicle-space-input">
            <label>Four Wheeler Spaces:</label>
            <input 
              type="number" 
              min="0"
              placeholder="Number of spaces" 
        ...
        {spacedetails.ParkingType.includes('Heavy Vehicle') && (
          <div className="vehicle-space-input">
            <label>Heavy Vehicle Spaces:</label>
            <input 
              type="number" 
              min="0"
              placeholder="Number of spaces" 
    ...
    <>
      <div className='add-space-container'>
        ...
        <form className='add-space-form' id='add-space-form'>
            ...
            {/* New section for vehicle space counts */}
            {spacedetails.ParkingType.length > 0 && (
              <div className="vehicle-spaces-section" style={{marginTop: '20px'}}>
                <label style={{marginTop:'20px', fontWeight:'bold'}}>Number of Spaces by Vehicle Type</label>
                {renderVehicleSpaceInputs()}
              </div>
            )}
